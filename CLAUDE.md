...
**entrypoint.py — 多 stage 启动流水线。** `run` 子命令按固定顺序执行 `scripts/sb_xray/stages/` 下的各 stage（keys / dhparam / geoip / nginx_auth / panels / supervisord / cron / xray_run 等），每个 stage 幂等且可用 `--skip-stage` 跳过。`entrypoint.py` 还提供一组非 `run` 子命令，多数是 supervisord 进程或 cron 入口：`show`、`trim`、`geo-update`、`isp-retest`、`substore-check`、`secrets-refresh`、`log-rotate`、`cert-renew`、`shoutrrr-forward`、`xray-run`、`xray-exit-listener`。
