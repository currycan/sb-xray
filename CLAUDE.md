...
业务逻辑（值得改动的代码）几乎全部是 `scripts/sb_xray/` 下的 Python 包，配置则是 `templates/` 下的 Jinja/占位符模板。运行时行为由 `docker-compose.yml` 的环境变量驱动，文档以编号 `docs/00–09` 为权威。
...
**文档权威在 docs/。** 编号 `00–09` 是正式项目文档（house style 见 project-docs 规范）：00 构建发布、01 架构与流量、02 协议与安全、03 路由与客户端、04 运维与排障（含 env 全集 / `CN_EXIT_MODE` 回国）、05 Reverse Proxy、06 事件总线、07 Tailscale、08 Xray Reverse Bridge、09 特性开关。改了行为要同步对应文档（见下方 §5）。
...
- **需要任何凭据 / secret / token / SSH / VPS 节点列表 / 域名** → 用 `project-credentials` skill，查 gitignore 的 `.credentials/` 目录，不要问用户或猜。
