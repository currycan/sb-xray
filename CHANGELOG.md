...
## [Unreleased] — M1–M4 大升级（底层 Xray v26.4.17）

> 本次升级横跨四个 milestone，覆盖 **可观测化 / 抗审查 / 内网穿透 / 单核收敛** 四条产品主线。 配套 52 条静态规约（`scripts/test_smoke.sh`）+ bracknerd 生产环境 E2E 验证通过。
>
...
**M2 · 抗审查（adv 已于 2026-04 并入主轨，三轨→两轨）**
...
- **feature flag `ENABLE_ECH`**（占位）：env 开关已注册，**TLS 层接入尚未实现**，启用暂无效果，预留给 M4-5b phase。
...
**M1–M4 · 跨横切**
- **`scripts/test_smoke.sh` 规约体系**：M1 8 条 + M2 4 条 + M3 5 条 + M4 5 条 + 通用静态检查，共 52 项；`SKIP_COMPOSE=1` 支持 CI 纯静态运行。
...
- **M1-1 · 源 IP 真实化**：`02_xhttp_inbounds.json` / `02_xhttp_compat_inbounds.json` / `03_vmess_ws_inbounds.json` 的 sockopt 添加 `"trustedXForwardedFor": ["X-Forwarded-For"]`（[Xray #5331](https://github.com/XTLS/Xray-core/pull/5331)）。nginx 前置写入的真实客户端 IP 不再被 xray 忽略，access log / webhook 事件的 source 准确。
- **M1-2 · DNS 抗故障**：`xr.json` 的 DNS 段启用 `enableParallelQuery: true` + `serveStale` 乐观缓存（[#5237](https://github.com/XTLS/Xray-core/pull/5237) / [#5239](https://github.com/XTLS/Xray-core/pull/5239)）。单 DNS 服务器故障不再硬等 4 秒，首次访问延迟显著降低。
- **M1-3 · 隐私合规**：`xr.json` log 段添加 `"maskAddress": "/16+/64"`（[#5570](https://github.com/XTLS/Xray-core/pull/5570)）。access log 里 IPv4 自动掩码前 16 bit、IPv6 前 64 bit。
- **M4 · 模板编号重排（最终形态）**：xhttp 家族全部归入 `02_` 前缀；emergency 后移保持紧凑。从 M4 初版（06/07/08/09）到当前状态的迁移轨迹：
  - `06_hy2_inbounds.json` → `04_hy2_inbounds.json`（Hy2 独立占 04，与 sing-box 家族对齐）
  - `07_xhttp_h3_inbounds.json` → **`02_xhttp_h3_inbounds.json`**（归入 02_xhttp_* 家族）
  - `08_xicmp_inbounds.json` → `05_xicmp_emergency_inbounds.json`（加 `_emergency_` 特征 + 滑位到 05）
...
- **M2-3 · v2rayN 导入 v2rayn-adv 订阅 10 节点中仅 Vmess-Adv 延迟 -1 失败**：根本原因是 VMess URL 标准不承载 Finalmask 字段，v2rayN UI 也未暴露 Finalmask 手动配置入口，客户端不发 SSH banner 与服务端握手对不上。**决策**：彻底删除 `templates/xray/03_vmess_ws_adv_inbounds.json` + nginx `/xxx-vmessws-adv` location；`V2RAYN_ADV_SUBSCRIBE` 不再包含 Vmess-Adv URL。触发未来重启的条件：XTLS/BBS discussions/716 把 `fm=` 字段推进到 vmess URL，或主流客户端 UI 暴露 Finalmask 手动配置。
- **smoke test M3-5 路径错误**：`docs/08-reverse-proxy-guide.md` → `docs/06-reverse-proxy-guide.md`（本次对齐附带修复）。

### Removed（移除）

- `templates/sing-box/01_hysteria2_inbounds.json`（Hy2 已迁至 Xray）
- `templates/xray/03_vmess_ws_adv_inbounds.json`（M2-3 决策，见 Fixed）
...
- `buildMphCache` CLI 调用 + `XRAY_MPH_CACHE` env 规划：PR #5505 被 upstream PR #5814 revert（2026-04-13），新方案是运行时自动生效的 matcher group 优化，无需 CLI / env。详见 `references/implementation-notes.md §M1-4`。
...
- `ENABLE_ECH` env 目前仅占位，启用后无实际效果。M4-5b phase 完成 TLS 层接入前，不建议在生产环境期待其行为。
...
- **生产部署模式**：从 `dockerhub pull latest` 切换到 "本地构建 + `docker save` + scp + `docker load` + 原地 `docker tag ... latest` + `docker compose up -d`" 可实现零公网镜像 tag 污染的滚动部署；参见本 release 周期 bracknerd 的验证流程。
...
- **实施笔记与字段探测记录**：[`references/implementation-notes.md`](./references/implementation-notes.md)（gitignored，本地维护）
- **上游能力挖掘清单**：[`references/xray-feature-opportunities.md`](./references/xray-feature-opportunities.md)（gitignored）

### 验证

- **静态规约**：`SKIP_COMPOSE=1 bash scripts/test_smoke.sh` → **52 通过 / 0 失败**
- **生产 E2E**（2026-04-21 bracknerd.ansandy.com）：
  ...
  - sing-box 作 Hy2 client 端到端握手 → `http=200 time=0.033s`，远端出口 IP 匹配 bracknerd 公网 IP
  - 订阅 URL 与 M3 完全一致（客户端无感迁移）

### 回滚（如遇问题）

```bash
# bracknerd 生产现场保留了回滚 tag
docker tag currycan/sb-xray:before-m4 currycan/sb-xray:latest
...
- `8230bde` — feat(M4): Xray single-backend convergence + experimental inbounds
