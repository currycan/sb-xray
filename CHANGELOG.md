...
- **OpenWrt 黄金备份恢复指引修正：配置 tar 可跨「同 target」设备恢复**（`docs/11-openwrt-rebuild-and-cutover.md` §4）：原指引「换了新硬件则走路径 A」对常见场景过度保守，实测推翻——从一台软路由经 LuCI「生成备份」（`sysupgrade -b`）导出配置 tar，导入到**同架构、仅 CPU/内存不同**的另一台软路由正常工作。根因：该备份只含 `/etc/config` 等配置文件、**不含内核/固件/驱动/设备树**，硬件驱动来自目标设备已刷好的固件；同一 OpenWrt target（如都为 `x86/64`）下 `board.json` 通用、网口命名一致，`/etc/config/network` 套得上，CPU/内存差异只影响性能不影响配置语义。§4 重写为三分边界（同机重建 / 同 target 换机可直接恢复 / 跨型号嵌入式走路径 A），并明确两条真实限制：Tailscale 身份单例（新旧两台同时在线会抢同一固定 IP，需老机先 `tailscale logout`）、跨型号嵌入式路由器（`board.json`/DSA 端口映射/无线芯片按型号绑定，`network`/`wireless` 可能对不上）。纯文档订正，不涉镜像/compose，无 watchtower 漂移契约（§2）影响。
...
- **OpenWrt tagged 节点重启后回国起不来（init auto-up 缺 `--advertise-tags`）**（`sources/openwrt/openwrt-init.sh`）：用 OAuth 登录的 tagged 节点（默认 `tag:openwrt`），其 `tailscale up` 每次都须带 `--advertise-tags`，否则被控制面拒、停在 `stopped`。生成的 `/etc/init.d/tailscale` 开机 auto-up 段、以及安装时 `tailscale up`（已登录后的幂等 re-up 分支）此前均未带 tag → 备份换机 / 断电重启后节点起来却不持固定 IP、回国腿无在线落地（2026-06-16 实战：黄金备份切换设备后回国中断，根因之一）。修复：新增 `_tagflag` / `_TS_TAGINIT`，**仅 OAuth 路径（`ts_has_oauth`）**给两处 up 注入 `--advertise-tags=$TS_OAUTH_TAGS`（普通 auth-key / 交互登录维持原样、不强加 tag 以免无 tag 授权失败）；init auto-up 在生成时静态烘焙进脚本。纯 OpenWrt 侧脚本，不涉镜像 / compose，无 watchtower 漂移契约（§2）影响。配套修正 `docs/11` §4「跨设备恢复边界」：配置备份不含 Tailscale 身份（state 在 `/etc/tailscale/`，不在 `sysupgrade -b` 默认范围），换机后须按 §13b/§15.3 重建身份并夺固定 IP。
