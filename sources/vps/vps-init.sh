...
#   REDEPLOY_URL            运行时更新脚本 sbx-redeploy.sh 下载源（拉到 ~/sb-xray）
...
#
# 运行时更新容器：跑 ~/sb-xray/sbx-redeploy.sh（重拉 compose + 重建容器 + 清理）。
...
运行时更新容器：跑 ~/sb-xray/sbx-redeploy.sh（本脚本已把它拉到 ~/sb-xray）。
...
load_config() {
    ...
    # 运行时更新脚本 sbx-redeploy.sh 的下载源（拉到 $SBXRAY_DIR，供日后更新容器）。
...
install_redeploy_helper() {
    # 把运行时更新脚本 sbx-redeploy.sh 拉到 $SBXRAY_DIR（所有节点；日后更新容器用）。
    # 下载→校验 shebang→替换；失败仅 warn（有本地副本则保留）。
    local f="$SBXRAY_DIR/sbx-redeploy.sh"
    if curl -fsSL "$REDEPLOY_URL" -o "$f.new" && [ -s "$f.new" ] && head -1 "$f.new" | grep -q '#!'; then
        mv "$f.new" "$f"; chmod 755 "$f"
        log "运行时更新脚本已就位 → $f"
    elif [ -f "$f" ]; then
        rm -f "$f.new"; warn "sbx-redeploy.sh 下载失败,保留现有 $f"
    else
        rm -f "$f.new"; warn "sbx-redeploy.sh 下载失败且本地缺失($REDEPLOY_URL)"
    fi
}
...
EOF
        ...
        if curl -fsSL "$CANARY_URL" -o "$_canary.new" && [ -s "$_canary.new" ] && head -1 "$_canary.new" | grep -q '#!/bin/sh'; then
            ...
            log "  sbx-canary-check.sh 已就位 ← $CANARY_URL"
        elif [ -f "$_canary" ]; then
...
EOF
    ...
    if [ "$SKIP_WATCHDOG_WIRING" = "1" ]; then
        ...
        if curl -fsSL "$WATCHDOG_URL" -o "$_wd.new" && [ -s "$_wd.new" ] && head -1 "$_wd.new" | grep -q '#!/bin/sh'; then
            ...
            log "cn-exit-watchdog.sh 已就位 ← $WATCHDOG_URL"
        elif [ -f "$_wd" ]; then
...
main() {
    ...
    log "=== 初始化完成，容器已启动。运行时更新容器跑：$SBXRAY_DIR/sbx-redeploy.sh ==="
    if [ -n "$OPENWRT_TS_IP" ]; then
