...
> **注意**：`uci add` 不幂等——以上命令重复执行会叠加出重复的 zone/forwarding。`openwrt/cn-exit-setup.sh` 已将本节固化为带探测守卫的幂等步骤，推荐用脚本执行；手动执行前先 `uci show firewall | grep tailscale` 确认不存在旧配置。
...
```yaml
- ENABLE_SOCKS5_PROXY=true         # SOCKS5 回国总开关（默认 true）；设 false 可一键停用本方案
- CN_EXIT_SOCKS5_HOST=100.x.x.x   # 替换为 tailscale ip 输出的 OpenWrt IP
- CN_EXIT_SOCKS5_PORT=7891         # OpenClash 默认 SOCKS5 端口
```

> `ENABLE_SOCKS5_PROXY=false` 时，即使 `CN_EXIT_SOCKS5_HOST` 有值也不会注入 SOCKS5 出站——临时停用方案一时不必清空 HOST，改这一个开关即可。
...
容器已经把下面这些参数全部填好，渲染出一份可以直接用的落地机配置。运行 `show` 看下载链接：

```bash
docker exec sb-xray show
```

输出里找到 `🔁 Reverse Bridge 落地机配置` 那一行，下面就是带 token 的下载地址，形如：

```
https://<你的域名>/sb-xray/reverse_bridge_client.json?token=<SUBSCRIBE_TOKEN>
```

把这个链接复制下来，下一步在 OpenWrt 上直接下载，**不用手动抄参数**。下面这张表只是帮你理解链接里都填了哪些值：
...
在 OpenWrt 上直接下载 §2.3 拿到的链接，占位符已经全部填好：
...
> 备选（不想用 token / 离线场景）：下载原始模板再手动替换。先 `wget https://raw.githubusercontent.com/currycan/sb-xray/main/templates/reverse_bridge/client.json -O /etc/xray/client.json`，把 §2.3 表格里的值**原样粘贴到命令行里回车**（变成 shell 变量），再执行：
> ```sh
> sed -i \
>   -e "s|\${DOMAIN}|${DOMAIN}|g" \
...
- **mihomo SOCKS 入站开了 authentication，但 `skip-auth-prefixes` 不含 Tailscale 段**：VPS 经 Tailscale 来的源 IP 是 `100.64.0.0/10`（CGNAT），若不在 skip-auth 豁免内，SOCKS 握手会被要求认证而失败（现象：本机经 `127.0.0.1:7891` curl 正常，但经本机 Tailscale IP 或从 VPS 测就失败）。`cn-exit-setup.sh` 已用 OpenClash overwrite 钩子幂等注入此段；手动修则在钩子里 `Value['skip-auth-prefixes'].unshift('100.64.0.0/10')` 后 `openclash restart`（reload 不触发 overwrite）
