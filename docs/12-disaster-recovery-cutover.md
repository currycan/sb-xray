...
- **CDN 优选**：服务自检一旦有硬失败（本阶段必有 Tailscale IP 那条），init 会打印 `已跳过 CDN 优选（优选需服务正常运行）` 并 `exit 1`——**这是设计内的**。CDN 优选**首跑**需 OpenClash/Tailscale 正常运行，故改到服务确认正常后（[§8b](#8b--补做-cdn-优选-ip服务确认正常后)）用 `sh openwrt-init.sh cdn` 补做；此阶段已落盘的只有优选工具 + 每日 cron。
...
### 8b · 补做 CDN 优选 IP（服务确认正常后）

📘 **做什么**：§8 已确认接管机 OpenClash / Tailscale / 回国出口全部正常——此刻才做 CDN 优选 IP。优选首跑会临时停 OpenClash 跑 CloudflareST 再恢复，**必须服务就绪才做**；全装（§7）因 Tailscale 临时身份自检失败而**有意跳过**了首跑，故在此显式补做。仍是态 A（现役机生产中、零影响）。
...
- [ ] **CDN 优选已生效**（§8b）：`/etc/hosts` 有「优选 IP → `子域名.CDN_DOMAIN`」条目（`sh openwrt-init.sh cdn status` 显示当前优选 IP）
