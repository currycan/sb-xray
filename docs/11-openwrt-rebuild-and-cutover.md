...
```
> 上面已默认走 `ghfast.top` 镜像适配国内；镜像失效时换其它 GitHub 代理前缀/去掉前缀直连，或在能访问 GitHub 的机器下载后传入（OpenWrt 无 sftp，`scp` 不行时用 `ssh root@<路由器IP> 'cat > /root/sb-xray-openwrt/openwrt-init.sh' < openwrt-init.sh`）。`cn-bridge` / `cn-bridge-monitor` 缺失不要紧，主脚本会自动经 `GH_PROXY` 镜像补下载。
...
🔧 **③ 装 OpenClash 本体 + Smart 核（socks5/balance 必需）**
```sh
sh openwrt-init.sh openclash      # 幂等装/更新 OpenClash 本体，并按 CPU 微架构自动装 mihomo Smart 核
```
> 核心按设备 CPU 自动选档（x86_64 依 `/proc/cpuinfo` 选 v3/v2/v1，arm64 用 linux-arm64），经 GitHub 镜像（`GH_PROXY`）下载到 `/etc/openclash/core/clash_meta` 并跑 `-v` 自检——CPU 不支持所选等级会报错并提示用 `CLASH_CORE_VARIANT` 手动降级。
...
🔧 **routine 化同机备份/恢复（`cn-backup`）**：上面是临时取备份手动恢复；日常自动留底用 `cn-backup`——复用同一 `sysupgrade` 机制，幂等补全 `/etc/sysupgrade.conf`（把 `/etc/cn-exit/`、`/root/sb-xray-openwrt/`、`/etc/init.d/xray-bridge-*` 等官方默认漏掉的有用路径纳入官方备份），每日 cron 产两份：`safe`（不含 Tailscale 身份）+ `full`（额外并入 `/etc/tailscale/`），本地明文留存、加密 `scp` 到 VPS、各留最近 N 份。同机回滚直接 `cn-backup restore <file>`（`.enc` 自动解密）。完整配置与用法见 [`../sources/openwrt/README.md`](../sources/openwrt/README.md) §5.9。

> ⚠️ `cn-backup restore` 走 `sysupgrade -r`，含 `network`/`firewall`，**远程执行可能自锁 SSH**；建议 console / 物理可达时操作。本节「整机重建 / 换机」仍按下方边界与 §5 身份重建走。
...
## 12. 确认 OpenClash 本体 + Smart 核就绪

📘 **做什么**：`CN_EXIT_MODE` 含 socks5 的模式需要 OpenClash 本体 + 可执行的 mihomo Smart 核。`openclash` 子命令两者一并装好（核心按 CPU 自动选档），全装流程也会在配置应用后补装/校验核心。

🔧 **怎么做 + ✅ 如何验证**（接管机）：
```sh
ls -l /etc/init.d/openclash             # 本体：存在且可执行
ls -l /etc/openclash/core/clash_meta    # Smart 核：存在
/etc/openclash/core/clash_meta -v       # 核可执行（打印版本即 CPU 兼容；报非法指令即架构不匹配）
```
> 本体或核心缺失/异常即补装（幂等，含按 CPU 自动装核）：`cd /root/sb-xray-openwrt && sh openwrt-init.sh openclash`。
