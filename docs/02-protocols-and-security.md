...
> **阅读路径**：订阅模型是所有协议的公共语境，先读 §1.1；所有 UDP 协议共享 ISP 高峰期 QoS 对策，集中在 §1.2。随后 §1.3–§1.9 按 Xray 系（星级降序 + XHTTP 服务端机制 §1.8 贴近 XHTTP 章节）展开，§1.10–§1.11 为 Sing-box 系，§1.12 为服务端落地代理附录，§1.13 为 XICMP/XDNS 应急通道机制，§1.14 为随机凭据生成机制。
...
### 1.13 应急通道机制：XICMP / XDNS

> 📘 **当 UDP/443 和 TCP/443 都被封时的最后通道。** 项目内置两个常态关闭的应急 inbound——XICMP（把代理数据藏进 ICMP echo）和 XDNS（把代理数据藏进 DNS 查询载荷），仅在常规端口全部失效时才有意义。**本节只讲机制**；如何打开、参数配置与连通性验证见 [docs/09 §6/§7](./09-feature-flags-and-capabilities.md)。

两者都是 **VLESS over mKCP** 的 finalmask 伪装通道，`decryption: none`（内层安全仍由 VLESS/Reality 外层负责），共用 `${XRAY_UUID}`：

| 维度 | 🔬 XICMP | 🔬 XDNS |
|:---|:---|:---|
| **入站模板** | `templates/xray/05_xicmp_emergency_inbounds.json` | `templates/xray/06_xdns_emergency_inbounds.json` |
| **inbound tag** | `XICMP_IN` | `XDNS_IN` |
| **承载层** | ICMP echo 报文（`finalmask.udp.type=xicmp`） | DNS 查询载荷（`finalmask.udp.type=xdns`，类 DNSTT） |
| **transport** | mKCP（`mtu 1400` / `tti 100` / up 10·down 20） | mKCP（`mtu 900` / **高 `tti 600`** 配合慢速 DNS 链路 / up 1·down 1） |
| **监听** | `::`，`port 0`（由 `finalmask` 接管，`id=${PORT_XICMP_ID}`） | `::`，`port ${PORT_XDNS}` |
| **特殊依赖** | 容器需 `cap_add=NET_RAW`（发原始 ICMP 包） | `${XDNS_DOMAIN}` 指向用户控制的 NS 域名（如 `tns.example.com`） |
| **何时有意义** | UDP/443 与 TCP/443 双双被封 | 同上；DNS 通常最后才被封，作为 XICMP 之后的兜底 |

```mermaid
flowchart LR
    User((客户端)) -- "UDP/TCP 443 全封?" --> Decide{常规通道<br/>可用?}
    Decide -- 可用 --> Normal["Reality / XHTTP / Hy2 …"]
    Decide -- "全封" --> Emerg{应急通道}
    Emerg -- "ICMP echo<br/>mKCP" --> XICMP(("XICMP_IN<br/>finalmask=xicmp"))
    Emerg -- "DNS 查询载荷<br/>mKCP 高 tti" --> XDNS(("XDNS_IN<br/>finalmask=xdns"))
    XICMP -- "VLESS 解封装" --> Internet((互联网))
    XDNS -- "VLESS 解封装" --> Internet
    class User entry
    class Decide,Emerg decision
...
> 这两条通道带宽极低（XDNS 上下行各 1 capacity）、延迟高，**只为"封到只剩 ICMP/DNS 还能出去"的极端场景保命**，不作日常使用。开关 `ENABLE_XICMP` / `ENABLE_XDNS` 默认关闭——详见 [docs/09 §6/§7](./09-feature-flags-and-capabilities.md)。

---

### 1.14 随机凭据生成（CSPRNG）

> 🔬 容器首次启动时，未显式注入的端口、UUID、订阅路径等敏感值由 `scripts/sb_xray/random_gen.py` 用 Python `secrets` 模块（CSPRNG，非 `random`）一次性生成并持久化，避免出厂固定值。

`scripts/entrypoint.py`（约 :338–361）按 `(env 名, 生成器)` 表逐项填充，仅当对应 env 未被外部设置时才生成：

| 类型 | 算法 | 取值范围 / 格式 | 典型用途 |
|:---|:---|:---|:---|
| `port` | `secrets.randbelow` | `[32000, 38000]` 数字串（含端） | `XUI_LOCAL_PORT` / `DUFS_PORT` |
| `uuid` | `uuid.uuid4()` | 标准 UUIDv4 | `XRAY_UUID` / `XRAY_REVERSE_UUID` / `SB_UUID` |
| `path` | `secrets.choice` ×N | N 个 `[a-z0-9]` 字符 | `XRAY_URL_PATH`（32）/ `SUBSCRIBE_TOKEN`（32） |
| `hex` | `secrets.token_hex` | `2×length` 个十六进制字符 | `XRAY_REALITY_SHORTID` / `_2` / `_3` |
| `password` | `secrets.choice` ×N | N 字符强口令 | `PASSWORD`（16） |

> 端口区间 `[32000, 38000]` 与历史 Bash 实现 `$(( RANDOM % 6001 + 32000 ))` 等价（`random_gen.py` 注释明确标注）。生成值写入 env 持久层后保持稳定，重启不重roll。
...
"decryption": "mlkem768x25519plus.native.600s.${XRAY_MLKEM768_SEED}"  // 防护 Nginx 中间节点
```
...
> [!IMPORTANT]
> **服务端 `decryption` 与客户端 `encryption` 字段非对称，不要互抄**：服务端 Xhttp 入站写 `native.600s.${XRAY_MLKEM768_SEED}`（`600s` 是 ticket 有效期、`SEED` 是服务端私钥种子，渲染自 `n768_n`）；客户端 URL（§2.4 / §1.5 / §1.6）写 `native.0rtt.${XRAY_MLKEM768_CLIENT}`（`0rtt` 是客户端握手模式、`CLIENT` 是公开配对密钥，渲染自 `n768_CLIENT`）。两侧握手模式与密钥角色都不同，是正常配对关系。
...
### 5.3 续期模型：启动时检查，无常驻守护进程

> 📘 **续期靠重启，不靠后台进程。** 容器内**没有**任何定时轮询证书的守护进程或 cron 任务；证书新鲜度只在**容器启动时**由 `scripts/sb_xray/cert.py::ensure_certificate` 检查一次。这是本项目证书续期的**单一真相源**，与 §5.5 的逐步流程一致。

* **触发时机**: 每次容器启动检查 `/pki/sb_xray_bundle.{crt,key,-ca.crt}` 三件套。
* **续期阈值**: 剩余有效期 **> 7 天**（`openssl x509 -checkend 604800`，`cert.py` 的 `_VALID_WINDOW_SECONDS = 7 * 24 * 3600`）→ `CertStatus.SKIPPED` 沿用现有证书；剩余 **≤ 7 天** 或任一文件缺失 → 全新签发并安装，随后重载 Nginx。
* **生效方式**: 因为只在启动时检查，长期运行的容器要让续期生效，需触达一次启动检查——通常由 `docker compose restart` 或常规重启触发；Let's Encrypt 证书 90 天有效，第 83 天起的下一次重启即会重签。
* **证书覆盖**: 泛域名证书 `*.example.com` + 主域名 `example.com`，由 `_build_issue_args` 自动为每个主域名追加 wildcard 一次签出。

> 完整的 CA 切换 / 立即重签 / 账户隔离行为见 §5.5。
...
### 5.5 CA 切换行为（`ACMESH_SERVER_NAME` 修改后发生什么）
