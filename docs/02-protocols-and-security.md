...
|:---|:---|:---|:---|:---|:---|:---|:---:|
| 1 | **XTLS-Reality** | Xray | TCP/443 | XTLS, Vision | v2rayn + compat | 日常主力，极速稳定 | ⭐⭐⭐⭐⭐ |
| 2 | **Hysteria2** | Xray | UDP/6443 | Salamander 混淆, QUIC | v2rayn + compat | 移动/弱网，暴力竞速 | ⭐⭐⭐⭐⭐ |
| 3 | **Xhttp-H3+BBR** | Xray | UDP/4443 | HTTP/3, BBR, ML-KEM | **仅 v2rayn** | UDP 直连极速，性能王 | ⭐⭐⭐⭐⭐ |
| 4 | **Xhttp+Reality直连** | Xray | TCP/443 | XHTTP, Reality, ML-KEM, adv+fragment | v2rayn + compat | 探索性主力 | ⭐⭐⭐⭐ |
| 5 | **上行Xhttp+TLS+CDN 下行Xhttp+Reality** | Xray | TCP/443 | XHTTP, 分裂路由 | v2rayn + compat | 上行走 CDN 隐藏客户端 IP，下行 Reality 直连 | ⭐⭐⭐⭐ |
| 6 | **上行Xhttp+Reality 下行Xhttp+TLS+CDN** | Xray | TCP/443 | XHTTP, 分裂路由 | v2rayn + compat | 上行 Reality 直连，下行走 CDN 分担服务器出口 | ⭐⭐⭐⭐ |
| 7 | **Xhttp+TLS+CDN 上下行不分离** | Xray | TCP/443 | XHTTP, TLS | v2rayn + compat | 上下行均经 CDN，服务器 IP 被封的保底 | ⭐⭐⭐ |
| 8 | **Vmess** (WS-TLS) | Xray | TCP/443 via CDN | WebSocket, CDN | v2rayn + compat | 传统兼容，救火 | ⭐⭐ |
| 9 | **TUIC** | Sing-box | UDP/8443 | QUIC | v2rayn + compat | Hysteria2 备选 | ⭐⭐⭐ |
| 10 | **AnyTLS** | Sing-box | TCP/4433 | TCP 指纹伪装 | v2rayn + compat | 企业级防火墙 | ⭐⭐⭐ |
...
### 1.1 订阅双轨：v2rayn vs v2rayn-compat
...
| `/v2rayn-compat` **兼容轨** | mihomo / Karing / OpenClash / 旧版 Xray-core（**< 26.3.27**） | `decryption=none` | 9 | 无（不含 H3） |
...
flowchart TD
    ...
    ClientType -->|mihomo / Karing / OpenClash / 旧版 Xray-core| Compat["/v2rayn-compat 兼容轨"]
...
- `v2rayn-compat` 牺牲 ML-KEM 和 H3 节点换兼容，TCP 轨的 adv obfs 字段在 compat 轨中也被剥离
...
* **订阅轨**: `v2rayn` + `v2rayn-compat` 双轨都含此节点
...
- **不支持** mihomo / Karing / OpenClash（均未实现 xhttp-h3 传输）→ 这些客户端请用 `/v2rayn-compat` 订阅
...
* **订阅轨**: `v2rayn` + `v2rayn-compat` 双轨都含此节点，但 compat 轨**剥离 ML-KEM 和 adv 字段**
* **内嵌抗 DPI 能力**:
...
* **客户端要求**: v2rayn 主轨 → Xray-core **≥ 26.3.27**；旧 core 握手失败请改用 `/v2rayn-compat`
...
* **URL 示例**:
  ...
  ```
* **服务端入站**: `templates/xray/02_xhttp_inbounds.json`（主轨，含 adv 字段）+ `02_xhttp_compat_inbounds.json`（compat 轨，decryption=none）
...
> **UDP QoS 对策**：见 §1.2；本协议抗 QoS 强度 ⭐⭐（仅标准 QUIC，依赖高位端口）。高峰期若 Hy2/H3 全部挂掉，客户端会自动切到 TUIC 或 TCP 轨（AnyTLS / Reality / XHTTP）。
...
| 旧版 Xray-core | < 26.3.27 | ❌ 握手失败 | `/v2rayn-compat`（**已剥离 adv 字段**） |
| mihomo / Karing / OpenClash | 未实现 xhttp 扩展 | ❌ | `/v2rayn-compat` |

v2rayn-compat 轨的 xhttp inbound 文件是 `02_xhttp_compat_inbounds.json`，**不含** adv 三字段、**不含** finalmask、**不含** ML-KEM（`decryption=none`），纯 TLS 兼容模式。
...
2. mihomo / Karing / OpenClash 本就走 `v2rayn-compat` 与 adv 无关
