...
- **通知质量优化(PoC 实测后定)**:① watchtower 容器设 `hostname: ${domain}`,标题显示节点域名(`Watchtower updates on dc99-3.ansandy.com`)而非容器 hash,16 台可辨;② `WATCHTOWER_NO_STARTUP_MESSAGE=true` 关掉每次重启的 startup 噪音,只在**真更新/失败**时发;③ watchtower 自带通知只负责「即时、朴素」,**详细更新报告交 §4.8 自检告警**(节点名+前后版本+健康结果,复用 `[sb-xray:$domain]` 风格),不上 watchtower 自定义模板(Go template 维护成本不值)。
