...
| `ENABLE_XICMP` / `ENABLE_XDNS` / `ENABLE_ECH` / `ENABLE_REVERSE` | `false` | 实验性 feature flag；开启条件与效果详见 [特性开关与可选能力指南](./09-feature-flags-and-capabilities.md) |
...
[选路] <路由判断结果>
[选路] IS_8K_SMOOTH: 出口=<tag> | 参考速度=XX.XX Mbps | 阈值=60 Mbps → <true/false>  → <标签提示>
...
| `IS_8K_SMOOTH → true → ✈ good 标签` | 代理均值 > 60 Mbps（`ISP_8K_SMOOTH_MBPS` 可调），节点将附加 good 标签 |
| `IS_8K_SMOOTH → false → 无质量标签` | 速度不足阈值，节点无 good/super 标签 |
...
| 节点速度均低于阈值（默认 60 Mbps） | `IS_8K_SMOOTH → false` | 正常现象，速度不足则无 good 标签 |
