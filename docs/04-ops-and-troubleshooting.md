...
| `DUFS_ALLOW_UPLOAD` | `false` | 允许上传文件（注：clap 布尔 `false` 被当「未提供」、被 `conf.yml` 的 `allow-all: true` 盖过 → 实际不生效，详见下方 dufs 深挖）；dufs 由 `-a` HTTP Basic Auth（`PUBLIC_USER`/`PUBLIC_PASSWORD`）把守，访问面收紧见 docs/02 §1.15 |
