...
2. [两轨订阅（v2rayn + common，adv 轨已并入主轨）](#2-两轨订阅v2rayn--commonadv-轨已并入主轨)
...
## 2. 两轨订阅（v2rayn + common，adv 轨已并入主轨）
...
- Karing / OpenClash 本就走 `common`（sing-box / mihomo 不认 ML-KEM）
- **三轨简化为两轨**：一套主轨吃最新 Xray-core，一套 common 吃其他
